...
interface StorySample {
  ...
  story: string;
  source_file: string;
  categories: string[];  // Changed from educational_categories
...
async function main() {
  console.log('🚀 Starting stories_sample_part04 import...');

  // Read JSON file from prisma directory
  const jsonPath = path.join(__dirname, 'stories_sample_part04.json');
  ...
  for (const story of stories) {
    try {
      ...
      // Determine age range based on difficulty and age_appropriateness
      let ageRange = '5-8';
      if (story.age_appropriateness === 'teen' || story.difficulty_level.includes('Middle School')) {
        ageRange = '9-12';
      } else if (story.difficulty_level.includes('High School')) {
        ageRange = '13-18';
      }
      ...
      const book = await prisma.book.create({
        data: {
          ...
          // Educational metadata (using "categories" field)
          educationalCategories: story.categories || [],
      ...
      console.log(`✅ Imported: "${book.title}" from ${story.country} (${story.categories.join(', ')})`);
      successCount++;
    } catch (error) {
