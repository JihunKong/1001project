...
interface BookManagerRevisionModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (data: BookManagerRevisionData) => void;
  isSubmitting?: boolean;
  submissionTitle: string;
}

export interface BookManagerRevisionData {
  notes: string;
  priority: 'LOW' | 'MEDIUM' | 'HIGH' | 'URGENT';
  dueDate?: string;
  revisionTypes: string[];
  targetRole: 'WRITER' | 'STORY_MANAGER';
}

const REVISION_TYPES = [
  { id: 'format', label: 'Format Issues', description: 'Story format needs adjustment for publication' },
  { id: 'length', label: 'Length Concerns', description: 'Story length needs modification' },
  { id: 'structure', label: 'Structural Changes', description: 'Story structure needs reorganization' },
  { id: 'content', label: 'Content Adjustment', description: 'Content needs revision for publication' },
  { id: 'illustrations', label: 'Illustration Needs', description: 'Needs illustration planning or modifications' },
  { id: 'metadata', label: 'Metadata Updates', description: 'Categories, tags, or age range need updates' },
];
...
const TARGET_OPTIONS = [
  ...
  { value: 'STORY_MANAGER', label: 'Send to Story Manager', description: 'Return to story manager for re-review' },
];

export default function BookManagerRevisionModal({
  isOpen,
  onClose,
  onSubmit,
  isSubmitting = false,
  submissionTitle
}: BookManagerRevisionModalProps) {
  const { t } = useTranslation();
  const [notes, setNotes] = useState('');
  const [priority, setPriority] = useState<'LOW' | 'MEDIUM' | 'HIGH' | 'URGENT'>('MEDIUM');
  const [dueDate, setDueDate] = useState('');
  const [revisionTypes, setRevisionTypes] = useState<string[]>([]);
  const [targetRole, setTargetRole] = useState<'WRITER' | 'STORY_MANAGER'>('WRITER');
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={handleClose}
      title={t('dashboard.bookManager.revisionModal.title')}
      size="lg"
    ...
    >
      <div className="space-y-6">
        <div className="bg-[#F9FAFB] border border-[#E5E5EA] rounded-lg p-4">
          <h3 className="text-sm font-medium text-[#141414] mb-1">{t('dashboard.bookManager.revisionModal.story')}</h3>
        ...
        <div>
          <label className="flex items-center gap-2 text-sm font-medium text-[#141414] mb-2">
            ...
            {t('dashboard.bookManager.revisionModal.sendTo')}
          </label>
          <div className="grid grid-cols-1 gap-2">
            {TARGET_OPTIONS.map((option) => (
              <button
                key={option.value}
                type="button"
                onClick={() => setTargetRole(option.value as 'WRITER' | 'STORY_MANAGER')}
        ...
        <div>
          <label className="flex items-center gap-2 text-sm font-medium text-[#141414] mb-2">
            ...
            {t('dashboard.bookManager.revisionModal.priority')}
          </label>
        ...
        <div>
          <label className="flex items-center gap-2 text-sm font-medium text-[#141414] mb-2">
            ...
            {t('dashboard.bookManager.revisionModal.dueDate')}
          </label>
          <input
            type="date"
            value={dueDate}
          ...
          <p className="text-xs text-[#8E8E93] mt-1">
            {t('dashboard.bookManager.revisionModal.dueDateHint')}
          </p>
        </div>

        <div>
          <label className="text-sm font-medium text-[#141414] mb-2 block">
            {t('dashboard.bookManager.revisionModal.revisionTypes')}
          </label>
        ...
        <div>
          <label className="text-sm font-medium text-[#141414] mb-2 block">
            {t('dashboard.bookManager.revisionModal.notes')} <span className="text-red-600">*</span>
          </label>
          <textarea
            value={notes}
            onChange={(e) => setNotes(e.target.value)}
            placeholder={t('dashboard.bookManager.revisionModal.notesPlaceholder')}
            rows={6}
            disabled={isSubmitting}
            className={`w-full px-4 py-3 border rounded-lg focus:outline-none focus:ring-2 focus:ring-soe-green-500 disabled:opacity-50 disabled:cursor-not-allowed ${
          ...
          <div className="flex items-center justify-between mt-2">
            <p className={`text-xs ${
              ...
              {charCount < 20
                ? t('dashboard.bookManager.revisionModal.charsNeeded', { count: 20 - charCount })
                : t('dashboard.bookManager.revisionModal.charCount', { count: charCount })}
            </p>
            {showValidation && !isValid && (
              <p className="text-xs text-red-600 flex items-center gap-1">
                <AlertCircle className="h-3 w-3" />
                {t('dashboard.bookManager.revisionModal.minChars')}
              </p>
            )}
          </div>
        </div>
        ...
        <div className="bg-soe-green-50 border border-soe-green-200 rounded-lg p-4">
          <div className="flex gap-3">
            ...
            <div className="flex-1 text-sm text-soe-green-700">
              <p className="font-medium mb-1">{t('dashboard.bookManager.revisionModal.guidelinesTitle')}</p>
              <ul className="space-y-1 text-xs">
                <li>• {t('dashboard.bookManager.revisionModal.guideline1')}</li>
                <li>• {t('dashboard.bookManager.revisionModal.guideline2')}</li>
                <li>• {t('dashboard.bookManager.revisionModal.guideline3')}</li>
                <li>• {t('dashboard.bookManager.revisionModal.guideline4')}</li>
              </ul>
            </div>
          </div>
        </div>
        ...
        <div className="flex gap-3 pt-2">
          ...
          >
            {t('dashboard.bookManager.revisionModal.cancel')}
          </button>
          <button
            type="button"
          ...
          >
            {isSubmitting ? t('dashboard.bookManager.revisionModal.submitting') : t('dashboard.bookManager.revisionModal.submit')}
          </button>
        </div>
      </div>
    </Modal>
