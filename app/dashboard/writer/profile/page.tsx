...
export default function WriterProfilePage() {
  ...
  useEffect(() => {
    if (status === 'unauthenticated') {
      router.push('/login?callbackUrl=/dashboard/writer/profile');
    }
  }, [status, router]);
  ...
  useEffect(() => {
    if (session?.user && activeTab === 'library') {
      fetchLibraryBooks();
    }
  }, [libraryFilter, activeTab]);

  const fetchLibraryBooks = async () => {
    try {
      const res = await fetch(`/api/user/library?filter=${libraryFilter}`);
      if (res.ok) {
  ...
  const fetchData = async () => {
    ...
    try {
      const [storiesRes, booksRes, profileRes] = await Promise.all([
        ...
        fetch(`/api/user/library?filter=${libraryFilter}`),
        fetch('/api/profile')
      ]);
      ...
      if (profileRes.ok) {
        ...
        setUserProfile({
          ...
          tags: profileData.profile?.tags?.map((tag: string, i: number) => ({
            id: String(i),
            name: tag,
  ...
  return (
    <div className="flex gap-8 bg-white min-h-[calc(100vh-80px)]">
      <div className="flex-1 px-8 py-10 max-w-[960px]">
        <h1
      ...
      <div className="hidden xl:block w-[320px] flex-shrink-0 border-l border-[#E5E5EA]">
        {userProfile && (
          <ProfileSidebar
            user={{
