    ...
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-8 flex justify-between items-center">
          ...
          <Link
            href={`/dashboard/${session?.user?.role?.toLowerCase().replace('_', '-')}/register-book`}
              ...
              {filteredBooks.map(book => (
                <div key={book.id} className="p-6 flex items-center gap-4 hover:bg-gray-50">
                  ...
                  <div className="flex-shrink-0 flex gap-2">
                    <button
                      onClick={() => router.push(`/dashboard/${session?.user?.role?.toLowerCase().replace('_', '-')}/books/${book.id}/edit`)}
