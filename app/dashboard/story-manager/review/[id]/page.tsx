...
export default function TextSubmissionReviewPage() {
  ...
  const fetchSubmissionDetail = async () => {
    try {
      setLoading(true);
      const response = await fetch(`/api/story-manager/text-submissions/${submissionId}`);
      if (response.ok) {
  ...
  const handleReviewAction = async (action: 'approve' | 'reject' | 'request_revision', feedback?: string, metadata?: any) => {
    try {
      ...
      const response = await fetch(`/api/story-manager/text-submissions/${submissionId}/review`, {
        method: 'POST',
        headers: {
  ...
  const handleFeedbackAdd = async (feedback: string, line?: number) => {
    try {
      const response = await fetch(`/api/story-manager/text-submissions/${submissionId}/feedback`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
            ...
            {processingAction && (
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-sm font-medium text-blue-900">Processing review action...</span>
                </div>
              </motion.div>
            )}
          </div>
